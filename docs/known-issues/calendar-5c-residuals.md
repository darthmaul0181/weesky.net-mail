...
- **L'atomicité de la bascule CalDAV et de la création de l'agenda `default`.** Le provider
  ...
  prouvent qu'à la main, sur `scotty_webmail_dev` — allumer CalDAV pour un compte sans agenda et
  voir naître ensemble la ligne `dav_credentials` et l'agenda `default` avec sa ligne
  `calendar_sync_state`. Procédure écrite dans `docs/schema-notes.md`, § « The sync counters ».
