...
`scotty_webmail` et `scotty_webmail_dev` :
...
vérifié à la main sur `scotty_webmail_dev` avant le premier déploiement, et la ligne est écrite
...
**Docs** : ce fichier ; `docs/schema-notes.md` (DDL 5c) ;
`docs/schema-notes.md` (le prérequis d'atomicité parle de `/caldav` : c'est
`/dav/calendars/`) ;
`docs/operations/restore-sync-epoch-rotation.md` (paragraphe agenda) ;
`docs/known-issues/calendar-5c-residuals.md` (créé en fin de tranche) ;
...
- **La transaction neuve de la bascule.** `DavCredentialStore.EnableAsync` n'en avait pas ; elle en
  ...
  Le filet est la vérification manuelle sur `scotty_webmail_dev` (§ 13).
