...
`trusted_senders` in `scotty_webmail`, on the pattern of the three tables already there: GUID FK
...
(`docs/schema-notes.md`) carries the script for **both**
`scotty_webmail` and `scotty_webmail_dev`, to be applied before the backend ships.
...
`docs/schema-notes.md` — the DDL, to run on `scotty_webmail` **and**
`scotty_webmail_dev` before the backend is deployed. Without the table every call answers 500 and
