...
`IAccountConnectionResolver` et la base webmail séparée (`scotty_webmail`) font que le cœur mail
...
| Packaging du plugin | **Ports fins + projet `scotty.providers.weesky`** | Petites interfaces par préoccupation, implémentées dans une class library qui référence le cœur — jamais l'inverse, le compilateur garde la frontière. Un binaire, activation par `"Platform"` dans appsettings. |
...
```
scotty.microservice            le cœur : hôte ASP.NET + tout le webmail générique
    ▲
    │ référence
scotty.providers.weesky        class library : tout ce qui touche la stack weesky.net
```

**Le cœur** : les quatre contrôleurs `api/Mail`, Contacts, Identities, Rules (credentials
utilisateur), Login (IMAP), quota utilisateur (IMAP), `PreferencesDbContext`/`scotty_webmail`,
...
- Les tests d'`Admin`/`Aliases`/`UsersRepository`/`DovecotQuotaClient` migrent vers
  `scotty.providers.weesky.Tests` ; le reste ne bouge pas
...
4. Aucune migration de données : `dovecot` et `scotty_webmail` gardent leurs schémas
