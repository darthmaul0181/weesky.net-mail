...
À rejouer sur `scotty_webmail` **et** `scotty_webmail_dev`. Création manuelle : ce projet n'utilise
pas les migrations EF. Le document de prérequis
Le DDL des contacts reçoit ces blocs comme il a reçu ceux
...
jusqu'à zéro. Il est documenté dans `docs/setup/` comme les autres prérequis de déploiement,
