...
Base `scotty_webmail`, DDL manuel sur le moule existant (deux bases, idempotent,
`utf8mb4_bin`, grants inchangés) : nouveau document
`docs/schema-notes.md`, à jouer avant déploiement.
...
3. DDL `webmail-connected-accounts-tables.md` joué sur `scotty_webmail` et
   `scotty_webmail_dev` avant déploiement.
