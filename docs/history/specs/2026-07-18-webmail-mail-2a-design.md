...
(`account-dev.mail.weesky.net`, service `scotty.microservice-dev`), ce qui donne un
...
```
/var/lib/scotty.microservice/keys/
...
systemd crée `/var/lib/scotty.microservice`, l'attribue au `User=` du service, le préserve
...
builder.Services.AddDataProtection()
    ...
    .SetApplicationName($"scotty.microservice.{builder.Environment.EnvironmentName}");
```
...
**Isolation dev / prod.** `scotty.microservice` et `scotty.microservice-dev` cohabitent sur le
...
appliquer aux **deux** unités (`scotty.microservice` et `scotty.microservice-dev`) avant le
...
`EnvironmentFile=/etc/scotty.microservice/secrets.env` est le mécanisme d'injection des
