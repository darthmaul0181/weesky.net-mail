...
| Stockage | Table `sending_identities` dans `scotty_webmail` — préférence webmail, jamais dans `dovecot` |
...
```sql
CREATE TABLE IF NOT EXISTS `scotty_webmail`.`sending_identities` (
...
`docs/schema-notes.md` sur le modèle des deux existants — script
