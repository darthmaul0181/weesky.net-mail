...
Le rapport de conformité est écrit sur le plan de son jumeau CardDAV.
Ce qui y entre est copié du fichier de `results/`, jamais de la
...
- `Services/Calendar/OccurrenceExpander.cs` / `Repositories/CalendarEventStore.cs` — une seule
  ...
  change pas (point 4).
- `scotty.microservice.Tests/Services/IcsTimeZonesTests.cs` — les alias `US/Eastern`,
...
- `scotty.microservice.Tests/Controllers/AppleDiscoveryReplayTests.cs` — nouveau.
...
- le rapport de conformité de la campagne.
- `docs/known-issues/calendar-5c-residuals.md` — chaque ligne que la campagne referme ou confirme.
- `docs/known-issues/calendar-5d-residuals.md` — le tri de fin de tranche, comme ses trois aînés.
