...
tables dans `scotty_webmail`, un CRUD, et un module frontend à trois colonnes — puis branche les
...
À rejouer sur `scotty_webmail` **et** `scotty_webmail_dev`. Un document de prérequis accompagne la
...
l'authentification et aux droits ; **aucun des quatre stores de `scotty_webmail`** —
