...
Une requête jouée sur `scotty_webmail_dev` le 2026-08-31 ne trouve aucune carte portant
...
À rejouer sur `scotty_webmail` **et** `scotty_webmail_dev`, avant tout déploiement du backend. À
verser dans `docs/schema-notes.md` comme les tranches précédentes.
...
**Un scénario client reste à jouer** (suivi dans `docs/known-issues/contacts-4e-groups.md`), et il ne peut pas être une
