...
- **Le piège du limiteur après une régénération**, que 4c-i n'a pas pu fermer et que ce plan ne ferme pas non plus : `AuthAttemptThrottle` est `internal` et un contrôleur public ne peut pas le prendre en paramètre. Le plan c pose la couture ou l'assume.
- **Le résidu de soixante secondes sur la révocation** : `Forget` ne peut pas battre un `Store` concurrent. Le fermer demande un compteur de génération dans `IDavAuthenticationCache`.
