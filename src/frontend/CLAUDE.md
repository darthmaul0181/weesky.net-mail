...
- `queries.ts` — TanStack Query hooks and keys; `api/mailTypes.ts` — response shapes. `useMessageStream` sets `refetchOnWindowFocus: false`: TanStack's default refetches *every* loaded page on focus, so forty loaded blocks would mean forty IMAP connections and forty full folder sorts on switching back to the tab. `App.tsx` sets that default to `true` app-wide, so an omission here would silently reintroduce it. `useFolders` polls independently — see below — and takes an `enabled` parameter so a caller that has no use for the tree, the notification hook among them, can leave the query off rather than duplicating it
...
**`refetchIntervalInBackground` (`queries.ts`, `useFolders`) is conditional on the settings, separately from `enabled`.** An unfocused tab that asked for nothing must keep costing nothing; a notification is only useful while the tab is elsewhere. `enabled` gates whether the query runs at all, `refetchIntervalInBackground` gates whether it keeps polling once the tab loses focus — a user with sound or desktop on needs the second, everyone else needs neither.
...
- `general/` — `GeneralPage.tsx` (messages per page, message-list preview, new-mail sound and desktop notification toggles)
...
Mail added role tokens across all four palette-mode blocks, for states a settings pane does not have: `--list-row-hover`, `--list-row-selected-bg`/`-fg`, `--list-separator`, `--badge-count-bg`/`-fg`, `--reader-header-border`, `--quote-text`, `--attachment-chip-bg`. `--accent-unread` was provisioned in the shell slice and has consumers only since the mail module. **Unread rows deliberately wear the selected tokens** (background, fg, inset bar) plus bold and the dot — same tokens rather than copied values, so the two looks cannot drift apart; the old `--list-row-unread-bg` was removed with its last consumer. What still singles out unread among selected-looking rows is the bold and the dot.
