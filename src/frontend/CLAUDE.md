...
**Two folder orders, deliberately different.** The mail column (`FolderTree`) floats the well-known folders to the top, because that is where a reader reaches for them. Everywhere the user hunts for a folder *by name* — the folders list, the create dialog's parent picker, the role selects — `sortFolders` puts the inbox first and then everything by name, system folders **interleaved** rather than grouped, so "Deleted Items" sits between "Courrier indésirable" and "Developpement". It compares with `localeCompare` (`sensitivity: 'base'`): a codepoint sort files every accented name after "Z", and a case-sensitive one exiles "e-commerce" past every capitalised name
