...
**Dark mode inverts the finished sheet** (`filter: invert(1) hue-rotate(180deg)`), the technique Apple Mail and Thunderbird use, rather than styling the body dark. **`contrast(0.74)` closes the chain**, because a straight inversion sends every white — the canvas and every mail that declares one — to pure black, a slab beside the app's soft greys; the value was tuned by rendering candidates against the palette's dark surfaces, and it cannot be derived from the token because `hue-rotate` is a matrix approximation that returns a tinted grey's complement visibly warm. **The filter sits on `html`, never on `body`**: a body background propagates to the canvas, which is painted *outside* body's own filter, so mail rendered as light-grey text on an untouched white sheet. `html` also needs a background of its own, since propagation only stops once it has one. Mail declares colours piecemeal — `color: #333` with no background is common — so a dark canvas turns that into dark on dark, while inverting whatever the message painted always preserves its contrast; the hue rotation keeps a red button red. **Only `img`/`video` are counter-inverted**: a rule for gradient backgrounds kept the Amazon navbar its navy, but any image nested in such a container inverted twice and came out negative. Since it is a rendering choice rather than a faithful one, `MessageReader` offers a per-message way back to the sender's colours, reset on every message like the image consent. The resolved theme comes from `useTheme().isDark` — the preference alone says nothing when it is `system`. None of it grants the body a capability; the sandbox is unchanged.
