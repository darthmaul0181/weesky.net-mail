...
| `AccountManagementController` (provider) | `/api/Account` | PATCH `ChangeSecret`, POST `FullName` | `[Authorize]` | Ships with `snoopy.providers.weesky`: both writes touch the platform's own directory, so they are routes only under `Platform: weesky`. `ChangeSecret` requires the old password. |
...
- `ConnectionStrings:WebmailPreferencesDatabase` — MySQL connection string to the `snoopy_webmail` database. Required on both platforms.
