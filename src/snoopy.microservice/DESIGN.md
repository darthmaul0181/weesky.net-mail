...
| `AccountController` | `/api/account` | GET, GET `Quota`, GET `Folders`, PATCH `ChangeSecret` | `[Authorize]` | `Quota` delegates to `IDoveadmClient`; `Folders` lists IMAP mailboxes (for the rules editor folder picker); `ChangeSecret` requires the old password and flushes the Dovecot auth cache for the mailbox. |
| `AliasesController` | `/api/aliases` | GET, POST, DELETE | `[Authorize]` | Every operation goes through `UserOwnsDomainAsync`. |
| `AdminController` | `/api/Admin/users`, `/api/Admin/domains`, `/api/Admin/ownerships`, `/api/Admin/flushcache` | GET, POST, PUT, DELETE | `[Authorize(Policy = "Admin")]` | User CRUD + quota proxy; domain CRUD; alias domain ownership GET/PUT/DELETE; POST `users/{id}/flushcache` and POST `flushcache` flush the Dovecot auth cache (per-user / global). The class-level admin policy (`AdminRequirementHandler` → `IsAdminAsync`) returns 403 for authenticated non-admins, 401 for anonymous callers. |
...
- `UsersRepository`: `FindByEmailAsync`, `IsValidPasswordAsync`, `GetAccountInfoAsync`, `ChangePasswordAsync`. Passwords are stored **plaintext** — the MariaDB `INSERT_PASSWORD`/`UPDATE_PASSWORD` triggers do the SHA-512 crypt encryption. After a successful password change it best-effort flushes the Dovecot auth cache via `IDoveadmClient` so the new password is effective immediately (a flush failure never fails the change).
...
- `DoveadmClient` (behind `IDoveadmClient`): typed `HttpClient` for the remote Dovecot server's `doveadm` HTTP API (header `Authorization: X-Dovecot-API <base64(ApiKey)>`). It POSTs `quotaGet` (result `STORAGE` + `MESSAGE` converted into a `Quota` DTO — bytes + message count, `0` = unlimited), `mailboxList` (IMAP folder names), and `authCacheFlush` (per-user `FlushAuthCacheAsync` / global `FlushAllAuthCacheAsync`). `HttpClient` timeout: 5s. Upstream errors are logged and bubble up as `Result.Failure` → the controller translates them into `502 Bad Gateway`.
