...
- `AdminController` — admin-only CRUD (class-level `[Authorize(Policy = AdminRequirement.PolicyName)]`; non-admins get **403**, unauthenticated callers 401): users (`GET/POST/PUT/DELETE /api/Admin/users`, `GET /api/Admin/users/{id}/quota`), domains (`GET/POST/PUT/DELETE /api/Admin/domains`), alias domain ownerships (`GET /api/Admin/ownerships`, `PUT /api/Admin/ownerships/{domainId}`, `DELETE /api/Admin/ownerships/{domainId}/{userId}`)
...
**Services** (`Services/`) wrap external integrations. `DoveadmClient` (behind `IDoveadmClient`, typed `HttpClient`) calls the remote doveadm HTTP API: `quotaGet` (live mailbox quota), `mailboxList` (IMAP folders), and `authCacheFlush` (`FlushAuthCacheAsync` per-user / `FlushAllAuthCacheAsync` global). `UsersRepository`/`AdminRepository` call it best-effort after password/quota/account mutations to invalidate Dovecot's auth cache (a flush failure never fails the DB operation). `ManageSieveClient` (behind `IManageSieveClient`) opens authenticated ManageSieve sessions (RFC 5804, port 4190, SASL PLAIN via a master user impersonating the mailbox) exposing list/get/put/setactive/delete script verbs.
