...
[ApiController]
public sealed class BearerAuthenticatorController : ApiBaseController
{
    ...
    public async Task<ActionResult<AuthToken>> Authenticate(Credentials credentials)
    {
        Result<AuthToken> result = await Authenticator.AuthenticateAsync(credentials.Email, credentials.Password);
