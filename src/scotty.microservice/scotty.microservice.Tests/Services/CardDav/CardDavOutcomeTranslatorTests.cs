...
using weesky.Scotty.Microservice.Services.CardDav;
using weesky.Scotty.Microservice.Services.Dav;
using weesky.Scotty.Microservice.Tests.Infrastructure;
using Xunit;

namespace weesky.Scotty.Microservice.Tests.Services.CardDav;

public sealed class CardDavOutcomeTranslatorTests
{
    ...
    public async Task AUidConflict_CarriesTheHrefOfTheHolder()
    {
        ...
        await CardDavOutcomeTranslator.WriteAsync(context.Response, outcome, CancellationToken.None);
    ...
    public async Task Busy_CarriesARetryAfter()
    {
        ...
        await CardDavOutcomeTranslator.WriteAsync(context.Response, Outcome(DavWriteStatus.Busy),
    ...
    public async Task AnAcceptedWrite_CarriesTheDavHeaderAndItsEtag(DavWriteStatus status)
    {
        ...
        await CardDavOutcomeTranslator.WriteAsync(context.Response,
    ...
    public async Task ANullEtag_StaysAbsentRatherThanEmpty()
    {
        ...
        await CardDavOutcomeTranslator.WriteAsync(context.Response, Outcome(DavWriteStatus.Created),
            CancellationToken.None);
    ...
    public void NoTwoStatuses_ShareTheirWholeAnswer()
    {
        ...
        var answers = Enum.GetValues<DavWriteStatus>()
            .Select(status => (Status: status, Code: CardDavOutcomeTranslator.StatusCodeOf(status),
                Condition: CardDavOutcomeTranslator.ConditionOf(status)))
            .ToArray();
    ...
    public async Task EveryEnumValue_IsHandled()
    {
        ...
        foreach (var status in Enum.GetValues<DavWriteStatus>())
        {
            ...
            var exception = await Record.ExceptionAsync(() =>
                CardDavOutcomeTranslator.WriteAsync(context.Response, Outcome(status), CancellationToken.None));
    ...
    public void EveryEnumValue_IsPinnedToItsOwnCodeByName()
    {
        ...
        var pinned = typeof(CardDavOutcomeTranslatorTests)
    ...
    public void InnoDbSayingComeBackLater_IsTransient(int mySqlErrorNumber) =>
        Assert.True(CardDavOutcomeTranslator.IsTransient(MySqlErrors.With(mySqlErrorNumber)));
    ...
    public void InnoDbSayingComeBackLater_IsTransientThroughEfsWrapper(int mySqlErrorNumber) =>
        ...
        Assert.True(CardDavOutcomeTranslator.IsTransient(
    ...
    public void AnyOtherMySqlNumber_IsNotTransient(int mySqlErrorNumber) =>
        Assert.False(CardDavOutcomeTranslator.IsTransient(MySqlErrors.With(mySqlErrorNumber)));
    ...
    public void AnOrdinaryDbUpdateException_IsNotTransient() =>
        ...
        Assert.False(CardDavOutcomeTranslator.IsTransient(new DbUpdateException("Duplicate entry")));

    [Fact]
    public void AnythingElse_IsNotTransient() =>
        Assert.False(CardDavOutcomeTranslator.IsTransient(new InvalidOperationException()));
