...
public sealed class CardDavPropertiesTests
{
        ...
        string kind, string ns, string localName)
    {
        ...
        var resolved = CardDavProperties.Resolve(Named(name), ContextFor(Kind(kind)));
    ...
    public void EachTable_DeclaresTheClosedSetAndNothingElse(string kind)
    {
        ...
        var declared = CardDavProperties
    ...
    public void TheServiceRoot_PointsAtThePrincipalAndClaimsNoResourceType()
    {
        ...
        Assert.Equal(DavPaths.Principal(UserId),
            HrefIn(Single(CardDavProperties.Resolve(Named(DavXml.Dav + "current-user-principal"), root))));
        Assert.Empty(Single(CardDavProperties.Resolve(Named(DavXml.Dav + "resourcetype"), root)).Elements());
    }
    ...
    public void TheHome_IsACollectionTheUserCanRead()
    {
        ...
        Assert.Equal("Address Books",
            Single(CardDavProperties.Resolve(Named(DavXml.Dav + "displayname"), home)).Value);
    }
    ...
    private static string Resolve(DavCard card, XName name) =>
        Single(CardDavProperties.Resolve(Named(name), CardContext(card))).Value;

    private static XElement ResolveCardElement(XName name) =>
        Single(CardDavProperties.Resolve(Named(name), CardContext(DefaultCard())));
    ...
    private static XElement ResolveCollectionElement(XName name) =>
        Single(CardDavProperties.Resolve(Named(name), CollectionContext()));

    private static (List<XElement> Found, List<XName> Missing) ResolveCollectionRequest(XName[] names) =>
        CardDavProperties.Resolve(new DavPropertyRequest(DavPropertyMode.Named, names), CollectionContext());

    private static (List<XElement> Found, List<XName> Missing) ResolvePrincipal(XName[] names) =>
        CardDavProperties.Resolve(new DavPropertyRequest(DavPropertyMode.Named, names), PrincipalContext());

    private static XElement ResolvePrincipalElement(XName name) =>
        Single(CardDavProperties.Resolve(Named(name), PrincipalContext()));
