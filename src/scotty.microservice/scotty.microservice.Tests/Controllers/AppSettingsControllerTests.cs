...
public sealed class AppSettingsControllerTests
{
    ...
    public async Task Set_StoresANameTrimmed()
    {
        await CreateController().SetAppSetting(
            new SetAppSettingRequest { Key = AppSettings.ShortName, Value = "  Scotty  " },
            CancellationToken.None);

        _store.Verify(s => s.SetAsync(AppSettings.ShortName, "Scotty", It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    [InlineData(AppSettings.ShortName, "Scotty webmail")]
