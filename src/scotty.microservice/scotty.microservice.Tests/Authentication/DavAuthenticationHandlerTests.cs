...
using weesky.Scotty.Microservice.Authentication;
...
using Xunit;
using DavSecret = weesky.Scotty.Microservice.Services.DavSecret;

namespace weesky.Scotty.Microservice.Tests.Authentication;

public sealed class DavAuthenticationHandlerTests
{
    ...
    private readonly Mock<ILogger<DavAuthenticationHandler>> log = new();
    ...
    public DavAuthenticationHandlerTests()
    {
        ...
        CancellationToken aborted = default)
    {
        ...
        var handler = new DavAuthenticationHandler(
        ...
        await handler.InitializeAsync(
            new AuthenticationScheme(DavAuthenticationDefaults.AuthenticationScheme, null,
                typeof(DavAuthenticationHandler)),
            context);
    ...
    private sealed class OptionsMonitorStub : IOptionsMonitor<DavAuthenticationOptions>
    {
        public DavAuthenticationOptions CurrentValue { get; } = new();
        public DavAuthenticationOptions Get(string? name) => CurrentValue;
        public IDisposable? OnChange(Action<DavAuthenticationOptions, string?> listener) => null;
    }
    ...
    public async Task AWrongSecret_Is401WithABasicChallengeAndNoBearerOne()
    {
        ...
        Assert.Equal($"Basic realm=\"{DavAuthenticationDefaults.Realm}\"", challenge);
    ...
    public async Task TheDavPolicyChallengesBasicAndOnlyBasic()
    {
        ...
        var policy = await services.GetRequiredService<IAuthorizationPolicyProvider>()
            .GetPolicyAsync(DavAuthenticationDefaults.PolicyName);
        ...
        Assert.Equal([DavAuthenticationDefaults.AuthenticationScheme], policy!.AuthenticationSchemes);
    }

    [Fact]
    public async Task TheDavSchemeIsRegisteredOnThisHandler()
    {
        var services = new ServiceCollection().AddLogging().AddScottyAuthentication().BuildServiceProvider();

        var scheme = await services.GetRequiredService<IAuthenticationSchemeProvider>()
            .GetSchemeAsync(DavAuthenticationDefaults.AuthenticationScheme);

        Assert.NotNull(scheme);
        Assert.Equal(typeof(DavAuthenticationHandler), scheme!.HandlerType);
    }
