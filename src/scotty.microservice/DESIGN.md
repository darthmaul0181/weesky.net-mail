# DESIGN — weesky.mail.scotty (scotty.microservice)
...
| `AccountController` | `/api/account` | GET, GET `Quota` | `[Authorize]` | `Quota` is read live over IMAP `GETQUOTAROOT INBOX` (200, or 204 when the server advertises no QUOTA capability) — no doveadm call. |
| `AccountManagementController` (provider) | `/api/Account` | PATCH `ChangeSecret`, POST `FullName` | `[Authorize]` | Ships with `scotty.providers.weesky`: both writes touch the platform's own directory, so they are routes only under `Platform: weesky`. `ChangeSecret` requires the old password. |
...
- `UsersRepository`: `FindByEmailAsync`, `GetAccountInfoAsync`, `ChangeFullNameAsync`, `ChangePasswordAsync`. It no longer validates credentials — login is an IMAP LOGIN attempt against the mail server, not a lookup here. Passwords are stored **plaintext** — the MariaDB `INSERT_PASSWORD`/`UPDATE_PASSWORD` triggers do the SHA-512 crypt encryption.
...
- `ManageSieveClient` (behind `IManageSieveClient`): opens an authenticated **ManageSieve** session (RFC 5804) to the mail server (TCP, default port 4190, STARTTLS). Authentication is SASL PLAIN with the account's own credentials — the same password held for its IMAP/SMTP session, never a master user. `OpenSessionAsync` returns an `IManageSieveSession` that exposes `ListScriptsAsync`, `GetScriptAsync`, `PutScriptAsync`, `SetActiveAsync`, and `DeleteScriptAsync` — the verbs `SieveRepository` drives. The session implements the line/literal framing of the protocol and surfaces server errors as `Result.Failure`. Connection settings come from `SieveOptions`.
...
`PreferencesDbContext` maps the separate `scotty_webmail` database, which holds webmail data rather than mail-server data. The contacts feature adds two tables to it:
...
- `Platform` — `weesky` or `generic`, at the root. Required: the service refuses to start without it. `generic` loads none of the weesky platform, so `api/Admin`, `api/aliases`, `PATCH api/Account/ChangeSecret` and `POST api/Account/FullName` are not routes at all — which holds only because the host csproj sets `GenerateMvcApplicationPartsAssemblyAttributes=false`; see the platform seam in `CLAUDE.md`.
- `ConnectionStrings:WebmailPreferencesDatabase` — MySQL connection string to the `scotty_webmail` database. Required on both platforms.
- `Weesky:ConnectionStrings:MailUserAccountsDatabase` — MySQL connection string to the `dovecot` database. Read only under `Platform: weesky`; **the root-level key of the same name is no longer read**.
- `TokenConstants` — Issuer, Audience, Key, ExpiryInMinutes, AuthCookieName.
- `Weesky:Dovecot:ApiUrl` — full URL of the remote `doveadm/v1` endpoint. `Weesky:Dovecot:ApiKey` — value of the `doveadm_api_key` shared with the service. Both moved under `Weesky` with the platform split, both required under `Platform: weesky`, and both validated on start: a missing or relative URL, or a blank key, refuses the boot naming the key rather than letting the admin quota column fail on its own later.
...
The calendar service account — the SMTP account that sends the invitation mails a CalDAV device's write owes the guests — is **not** configuration. An administrator enters it in Administration > Application (`api/SchedulingAccount`); it lives in the single-row `scheduling_service_account` table (`docs/schema-notes.md`), its password protected by Data Protection under a purpose of its own. A `Scheduling` section or `Scheduling__Smtp__*` variables left over from an earlier deployment are ignored. Without an account, or with a password the key ring can no longer decrypt, those mails are refused and logged, never sent. The single retry waits a fixed minute. **This assumes a single instance of the service.** The account is cached in the process (`ServiceAccountProvider`) and only the instance that saved or deleted it reloads it; a second instance would keep sending with the old account, or refusing without one, until it restarts. The one-connection-test-at-a-time gate is a static semaphore of the process too, so two instances would each run a test of their own.

### Delivery door

`POST /api/Delivery/CalendarReplies` lets Dovecot's Sieve script apply a guest's REPLY into a mailbox's calendar at delivery, without a session. The switch is deliberately **not** in `app_settings` — that table is read anonymously (`GET /api/AppSettings` has no auth, the manifest needs it) and would tell all of Internet whether the door is open — it lives in the `delivery_reply_key` row instead, and a wrong key answers exactly the same bare 404 as a disabled switch (spec 5e3, décision 7). What the key can do is narrow by design: it can name any mailbox, but can only rewrite the `PARTSTAT` and timestamp of an `ATTENDEE` line already present on an event the webmail itself organizes — never create, delete, move, or touch an event received from someone else (décision 12). The `delivery` rate-limit policy is a concurrency limiter (8 in flight, 16 queued, 503 past that), not an address-partitioned one: every legitimate caller is the one mail server, so a per-address bucket would just be a single global one anyone could drain (décision 13). See `docs/history/specs/2026-09-14-webmail-calendar-5e3-delivery-replies-design.md` and `install/optional/delivery-replies.md`.
